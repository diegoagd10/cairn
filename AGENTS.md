...
Use `src/store.ts` as the shared domain boundary. CLI and HTTP changes use it. The UI may edit document titles, Markdown bodies, and status, and permanently delete documents in bulk, through same-origin, revision-checked endpoints; other edits use the CLI. Project data belongs in the user's data directory, outside Git. Resolve worktrees through their common Git directory and clones through a normalized remote.
